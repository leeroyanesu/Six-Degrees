...
const JoystickComponents = (props: EcctrlJoystickProps) => {
  ...
  const onTouchMove = useCallback(
    (e: TouchEvent) => {
      ...
      const runState =
        joystickDis >
        joystickMaxDisRef.current * (props.joystickRunSensitivity ?? 0.9);
  ...
  useEffect(() => {
    ...
    joystickHalfWidthRef.current =
      joystickDiv.getBoundingClientRect().width / 2;
    joystickHalfHeightRef.current =
      joystickDiv.getBoundingClientRect().height / 2;

    joystickMaxDisRef.current = joystickHalfWidthRef.current * 0.65;

    joystickCenterXRef.current =
      joystickPositionX + joystickHalfWidthRef.current;
    joystickCenterYRef.current =
      joystickPositionY + joystickHalfHeightRef.current;
